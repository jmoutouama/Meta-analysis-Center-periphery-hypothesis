...
```{r}
# Extract VarCorr object
vc <- VarCorr(Lambdamodel)
# Get posterior mean of each variance component
sigma_ID    <- as.numeric(vc$ID$sd)^2        # study-level
sigma_Species <- as.numeric(vc$Species$sd)^2 # species-level
sigma_resid <- as.numeric(vc$residual__sd)^2 # residual

# Total variance
sigma_total <- sigma_ID + sigma_Species + sigma_resid

# Compute % contribution (I²-like)
I2_ID      <- 100 * sigma_ID / sigma_total
I2_Species <- 100 * sigma_Species / sigma_total
I2_resid   <- 100 * sigma_resid / sigma_total

# Display results
data.frame(
  Component = c("ID", "Species", "Residual"),
  PercentVariance = c(I2_ID, I2_Species, I2_resid)
)
```
...
# Function to compute tau², I², and variance partitioning
compute_heterogeneity <- function(fit, se_vector = NULL, group_names = c("ID", "Species", "residual")) {
  
  # Extract posterior draws
  post <- as_draws_df(fit)
  
  # Extract variance components (replace names if your model uses different labels)
  ...
  tau2_total <- tau2_ID + tau2_Species + sigma2
  
  # Variance partitioning
  h_list <- list()
  if("ID" %in% group_names) h_list$ID <- 100 * tau2_ID / tau2_total
  if("Species" %in% group_names) h_list$Species <- 100 * tau2_Species / tau2_total
  if("residual" %in% group_names) h_list$residual <- 100 * sigma2 / tau2_total
  
  # Compute I² if standard errors provided
  I2 <- NULL
  if(!is.null(se_vector)) {
    mean_se2 <- mean(se_vector^2)
    I2 <- 100 * tau2_total / (tau2_total + mean_se2)
  }
  
  # Summarize posterior distributions
  tau2_summary <- describe_posterior(tau2_total, centrality = "all", ci = 0.95)
  I2_summary <- if(!is.null(I2)) describe_posterior(I2, centrality = "all", ci = 0.95) else NULL
  h_summary <- lapply(h_list, function(x) describe_posterior(x, centrality = "all", ci = 0.95))
  
  return(list(
    tau2 = tau2_summary,
    I2 = I2_summary,
    variance_partitioning = h_summary
  ))
}
...
```{r}
Lambdamodel_i <- compute_heterogeneity(Lambdamodel, se_vector = datlambda$se)

# Access results
Lambdamodel_i$tau2
Lambdamodel_i$I2
Lambdamodel_i$variance_partitioning$ID
Lambdamodel_i$variance_partitioning$Species
Lambdamodel_i$variance_partitioning$residual
...
```{r}
genamodel_i <- compute_heterogeneity(genamodel, se_vector = datgencp$se)
genamodel_i$tau2
genamodel_i$I2
genamodel_i$variance_partitioning$ID
genamodel_i$variance_partitioning$Species
genamodel_i$variance_partitioning$residual
```
...
```{r}
abunmodel_i <- compute_heterogeneity(abunmodel, se_vector = datgencp$se)
abunmodel_i$tau2
abunmodel_i$I2
abunmodel_i$variance_partitioning$ID
abunmodel_i$variance_partitioning$Species
abunmodel_i$variance_partitioning$residual
```
...
(Figure6<-out_all_method %>%   
  ...
  # Add text labels
  geom_text(size = 2.8,
    data = mutate_if(out_all_sum_method, is.numeric, round, 3),
